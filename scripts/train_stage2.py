"""Stage 2 training entry point: dynamics + decoder training, frozen encoders."""
...
from iome.train.stage2    import Stage2DynamicsModule
...
def parse_args():
    ...
    ap.add_argument("--p_mod_drop",  type=float, default=0.2)
...
def main():
    ...
    state   = {k.removeprefix("model."): v for k, v in s1_ckpt["state_dict"].items()}
    missing, _ = model.load_state_dict(state, strict=False)
    if missing:
        print(f"[stage2] missing keys from Stage 1 ckpt: {missing[:5]}...")

    module = Stage2DynamicsModule(
        model=model,
        max_steps=args.max_steps,
        p_mod_drop=args.p_mod_drop,
    )

    logger  = WandbLogger(project=args.wandb_project, entity=args.wandb_entity, name="stage2")
    ckpt_cb = ModelCheckpoint(
        dirpath=str(args.ckpt_dir),
        filename="stage2-{step:06d}-{val/loss:.4f}",
